...
import { FolderOpen, Plus, Trash2, Clock, Image as ImageIcon, X, ArrowRight } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-4 pr-8">
          <div>
            ...
            <p className="text-xs text-neutral-400">
              Autosaved collages in your local and cloud storage workspace.
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-gradient-to-r from-indigo-500 to-pink-500 hover:from-indigo-600 hover:to-pink-600 text-white text-xs font-bold rounded-xl shadow-md cursor-pointer transition-all shrink-0"
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {projects.map(proj => (
                <div
                  ...
                  className="p-4 rounded-xl bg-neutral-950/80 border border-neutral-800 hover:border-indigo-500/50 hover:bg-neutral-950 transition-all cursor-pointer group flex flex-col justify-between"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="font-bold text-sm text-white group-hover:text-indigo-400 transition-colors truncate max-w-[180px]">
                      {proj.name}
                    </div>
                    <button
                      onClick={e => handleDelete(e, proj.id)}
                      className="p-1.5 text-neutral-500 hover:text-rose-400 hover:bg-rose-950/50 rounded-lg transition-colors"
